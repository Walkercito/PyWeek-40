...
class UIManager:
    ...
    def draw_credits_screen(self, skybox, menu_camera, credits, button_rects, hover_states, link_hover_states):
        self.draw_rotating_skybox(skybox, menu_camera)
        sw, sh = get_screen_width(), get_screen_height()
        title_font_size = int(80 * sh / 980)
        ...
        for i, credit in enumerate(credits):
            draw_text_ex(self.font, f"{credit['name']}:", Vector2(int(sw * 0.1), y_pos), name_font_size, 1, SKYBLUE)
            draw_text_ex(self.font, credit['reason'], Vector2(int(sw * 0.1 + 20), int(y_pos + name_font_size * 1.2)),
                         text_font_size, 1, WHITE)
            if credit.get('link'):
                if link_index < len(link_hover_states):
                    is_hovering = link_hover_states[link_index]
                    link_color = ORANGE if is_hovering else YELLOW
                    draw_text_ex(self.font, credit['link'],
                                 Vector2(int(sw * 0.1 + 20),
                                         int(y_pos + name_font_size * 1.2 + text_font_size * 1.2)), text_font_size,
                                 1, link_color)
                    link_index += 1
            y_pos += int(sh * 0.15)
